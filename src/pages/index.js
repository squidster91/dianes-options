        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold text-white">Put Options Scanner</h1>
          ...
          {!data && !loading && !error && (
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-12 text-center">
              ...
              <p className="text-slate-400">Enter a ticker symbol and click Analyze to fetch live options data</p>
            </div>
          )}
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                  ...
                  <div className="text-white text-2xl font-bold">{data.ticker}</div>
                </div>
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                  <div className="text-slate-400 text-xs uppercase tracking-wide mb-1">Price</div>
                  <div className="text-white text-2xl font-bold">${data.currentPrice?.toFixed(2)}</div>
                  {data.priceChange && (
                    <div className={`text-sm ${data.priceChange?.startsWith('-') ? 'text-red-400' : 'text-emerald-400'}`}>
                      {data.priceChange}
                    </div>
                  )}
                </div>
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                  ...
                  <div className="text-white text-lg font-bold">{data.expiration}</div>
                </div>
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                  <div className="text-slate-400 text-xs uppercase tracking-wide mb-1">Days to Expiry</div>
                  <div className={`text-2xl font-bold ${data.daysToExpiry <= 3 ? 'text-amber-400' : 'text-white'}`}>
                    {data.daysToExpiry}
                  </div>
                </div>
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                  <div className="text-slate-400 text-xs uppercase tracking-wide mb-1">Avg IV</div>
                  <div className={`text-2xl font-bold ${data.avgIV > 50 ? 'text-purple-400' : 'text-white'}`}>
                    {data.avgIV}%
                  </div>
                </div>
                <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                  <div className="text-slate-400 text-xs uppercase tracking-wide mb-1">Earnings</div>
                  <div className={`text-lg font-bold ${data.hasEarnings ? 'text-red-400' : 'text-emerald-400'}`}>
                    {data.hasEarnings ? '⚠️ YES' : '✓ Clear'}
                  </div>
                </div>
              </div>
              ...
              {/* Full Options Table */}
              {data.allPuts && data.allPuts.length > 0 && (
                <div>
